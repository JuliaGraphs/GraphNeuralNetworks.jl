...
- **Changelog**: there is a single repo-wide `CHANGELOG.md` at the root, split into one section per package. Any user-facing change (new feature, bug fix, deprecation, or compat relaxation) must add an entry under the relevant package's latest version, in [Keep a Changelog](https://keepachangelog.com/) style, linking the PR (`[#NNN]` reference at the bottom of the file). Skip purely internal churn (tests, CI, docs, refactors). When tagging a release, turn the package's `Unreleased`/`-DEV` heading into a dated `X.Y.Z — YYYY-MM-DD` heading.
