...
@testitem "TGCNCell" setup=[TemporalConvTestModule, TestModule] begin
    ...
    test_gradients(cell, g, g.x, loss=cell_loss, rtol=RTOL_HIGH, ad_backends = AD_BACKENDS)
    # with initial state
    test_gradients(cell, g, g.x, h, loss=cell_loss, rtol=RTOL_HIGH, ad_backends = AD_BACKENDS)
    ...
    # with no initial state
    test_gradients(cell_custom, g, g.x, loss=cell_loss, rtol=RTOL_HIGH, ad_backends = AD_BACKENDS)
    # with initial state
    test_gradients(cell_custom, g, g.x, h_custom, loss=cell_loss, rtol=RTOL_HIGH, ad_backends = AD_BACKENDS)
end
...
@testitem "TGCN" setup=[TemporalConvTestModule, TestModule] begin
    ...
    # with no initial state
    test_gradients(layer, g, x, rtol = RTOL_HIGH, ad_backends = AD_BACKENDS)
    # with initial state
    test_gradients(layer, g, x, state0, rtol = RTOL_HIGH, ad_backends = AD_BACKENDS)
    ...
    @test !isapprox(y, y_custom, rtol = RTOL_HIGH)
    # with no initial state
    test_gradients(layer_custom, g, x, rtol = RTOL_HIGH, ad_backends = AD_BACKENDS)
    # with initial state
    test_gradients(layer_custom, g, x, state0, rtol = RTOL_HIGH, ad_backends = AD_BACKENDS)

    # interplay with GNNChain
    model = GNNChain(TGCN(in_channel => out_channel), Dense(out_channel, 1))
    y = model(g, x)
    @test size(y) == (1, timesteps, g.num_nodes)
    test_gradients(model, g, x, rtol = RTOL_HIGH, atol = ATOL_LOW, ad_backends = AD_BACKENDS)
end
...
@testitem "EvolveGCNO" setup=[TemporalConvTestModule, TestModule] begin
    ...
    @test size(y) == (out_channel, timesteps, g.num_nodes)
    # with no initial state
    test_gradients(layer, g, x, rtol=RTOL_LOW, atol=ATOL_LOW, ad_backends = AD_BACKENDS)
    # with initial state
    test_gradients(layer, g, x, state0, rtol=RTOL_LOW, atol=ATOL_LOW, ad_backends = AD_BACKENDS)

    # interplay with GNNChain
    model = GNNChain(EvolveGCNO(in_channel => out_channel), Dense(out_channel, 1))
    y = model(g, x)
    @test size(y) == (1, timesteps, g.num_nodes)
    test_gradients(model, g, x, rtol=RTOL_LOW, atol=ATOL_LOW, ad_backends = AD_BACKENDS)
end
